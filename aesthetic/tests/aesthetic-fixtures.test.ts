...
import { checkProgram, constraintsOf, loadAestheticProgram, validateAestheticProgram } from '../check.js';
...
function pureAestheticSources(dir = path.join(ROOT, 'aesthetic')): string[] {
  ...
  for (const entry of readdirSync(dir, { withFileTypes: true })) {
    // `aesthetic/tests/` is not the layer. It renders, so it imports the browser on purpose, and
    // counting it here would make the pure half look impure by the act of checking it.
