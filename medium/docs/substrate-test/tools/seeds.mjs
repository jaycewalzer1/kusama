...
//
// Run from the medium/ directory:  node docs/substrate-test/tools/seeds.mjs
...
if (process.argv[1]?.endsWith('seeds.mjs')) {
  const outDir = 'out/substrate-test/seeds';
  mkdirSync(outDir, { recursive: true });
  for (const probe of PROBES) {
    const program = JSON.parse(readFileSync(path.join('examples/probes', `${probe}.json`), 'utf8'));
