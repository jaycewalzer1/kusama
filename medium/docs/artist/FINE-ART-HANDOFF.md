...
1. **The medium** (`medium/env/`, `medium/renderer/`) — a deterministic, edit-based drawing
...
2. **The aesthetic layer** (`medium/aesthetic/`) — *positions* (an artistic practice, with
...
3. **The artist layer** (`medium/artist/`) — the loop that puts a language model in front of all of
...
4. **The studio** (`medium/cli/ui.ts` + `medium/ui/`, port 4321) — a web console over layer 3:
...
3. `npm --prefix ./medium run test` — expect all tests green and all 11 golden pixel hashes unmoved.
   A moved golden means the medium changed, which the catalog must never do.
4. `npm --prefix ./medium run ui -- --runs out --port 4321` and author or audit through the studio;
