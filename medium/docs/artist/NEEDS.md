...
**The base position these tests compose against is a frozen fixture, so the pack's fit is unproven.**
`aesthetic/elements/` was built against the position `cut-and-reset`, and the catalogue was rebuilt
out from under it in the same session that added the finish gate: `cut-and-reset`, `data-austerity`
and `generation-loss` are gone, and `interference`, `many-hands` and `withheld` are what a run can
actually be commissioned on. Every fixture in `tests/elements.test.ts` is a property of the old
position — twelve constraints, `c-cut-blocks`, `p-no-illustration`, and above all
`rodchenko-red-black` composing to **zero** conflicts, which is the only clean-compose case in the
suite. Rather than re-derive them against a position the pack was not selected for, the old document
