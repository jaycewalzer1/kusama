...
export const ROOT = (() => {
  ...
  for (let i = 0; i < 6; i++) {
    if (existsSync(path.join(dir, 'vendor', 'p5.min.js'))) return dir;
    dir = path.dirname(dir);
  }
  throw new Error('cannot locate the medium root (vendor/p5.min.js not found above this file)');
})();
...
export class Renderer {
  ...
  static async launch(): Promise<Renderer> {
    ...
    for (const [name, rel] of Object.entries(FONT_FILES)) fonts[name] = await sha256(path.join(ROOT, rel));
    const manifest: RendererManifest = {
      p5: { version: '2.2.0', sha256: await sha256(path.join(ROOT, 'vendor/p5.min.js')) },
      p5brush: { version: '2.1.0-beta', sha256: await sha256(path.join(ROOT, 'vendor/p5.brush.js')) },
      browser: {
        name: 'chromium',
  ...
  private async serve(route: Route): Promise<void> {
    ...
    try {
      const body = await readFile(path.join(ROOT, safe));
