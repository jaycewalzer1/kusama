...
cli.action(async (opts: { examples: string; goldens: string; profile?: string; update?: boolean; check?: boolean }) => {
  ...
  // directory can hold v0 and v1 goldens side by side and both stay checked.
  const jobs = files.map((file) => {
    const program = JSON.parse(readFileSync(path.join(opts.examples, file), 'utf8')) as { assetPack?: string };
    const { profile, hash: profileHash } = loadProfileFor(program, opts.profile);
    ...
    return { name: path.basename(file, '.json'), program, pack, profileHash, resolved, fonts: fontsUsed(resolved) };
  });
  ...
  try {
    for (const job of jobs) {
      ...
      // Same order as `render`: print after the repeat loop has already decided this render is real.
      const rgba =
        job.resolved.print.length > 0
          ? runPrint(out.rgba, out.width, out.height, job.resolved.print, job.resolved.seed).rgba
          : out.rgba;
      entries[job.name] = {
        programHash: contentHash(job.program),
        packHash: job.pack.hash,
        profileHash: job.profileHash,
        pixels: pixelHash(rgba),
      };
      images[job.name] = encodePng(rgba, out.width, out.height);
    }
  } finally {
