...
import { loadPack, loadPackFor, packHash, PackError, type Face, type License } from '../pack.js';
import { loadProfile, loadProfileFor, ProfileError } from '../profile.js';
import { validateProgram } from '../validate.js';

const manifest = JSON.parse(readFileSync(path.join(ROOT, 'renderer/assets/fonts/manifest.json'), 'utf8')) as {
...
/**
 * Where a pack-declared path lives on disk. The packs store their faces as `assets/fonts/...` and
 * the pack hash is taken over those strings, so the paths stayed put when the files moved under
 * `renderer/`. `env/browser.ts` resolves them the same way for the hermetic route.
 */
function onDisk(file: string): string {
  return path.join(ROOT, 'renderer', file);
}

function sha256(file: string): string {
  return createHash('sha256').update(readFileSync(onDisk(file))).digest('hex');
}
...
test('every vendored face on disk is byte-for-byte the file the manifest declares', () => {
  ...
  for (const [name, face] of Object.entries(manifest.faces)) {
    assert.equal(statSync(onDisk(face.file)).size, face.bytes, `${name}: file size does not match the declared bytes`);
